...
public class SSChecckDataThread extends  Thread{
    ...
    public void run() {
        ...
        if(StringUtils.isEmpty(sorsefilepath)||StringUtils.isEmpty(savebasepath)){
            System.out.println(sorsefilepath+":sorsefilepath=====有一个参数为空====savebasepath:"+savebasepath);
        ...
        System.out.println(savebasepath+"--savebasepath filename："+filename);

        while(bool){

            if(!bool){
                System.out.println("SSChecckDataThread 被主动中止--filename："+filename);
            ...
            try {
                ...
                if(null!=savefilepathlist&&savefilepathlist.size() > 0){
                    ...
                    for(int i=size-1;i>=0;i--){//从最后面查起
                        ...
                        System.out.println(path+":path 路径对比 filename:"+filename);
                        if(path.endsWith(filename)){//当找到指定文件后进行大小比较

                            File savefile=new File(path);
                            long filerealsize=savefile.length();
                            System.out.println(filerealsize+":filerealsize--filename:"+filename+"--filesize_db："+filesize_db);
                            if(filerealsize==filesize_db){//判断文件是否全部上传完毕
                                ...
                                System.out.println(ss_databaseupdateById+":ss_databaseupdateById----SSChecckDataThread ss_dataMessageParam.getId()："+ss_dataMessageParam.getId());
                                bool=false;//判断完了就出去了
                            }
                            break;//找到了文件就不需要再找了
                        }
            ...
            if(!bool){
                System.out.println("SSChecckDataThread 被主动中止,可能是已经完成了--filename："+filename);
                break;
            }
        ...
        System.out.println("正常结束检测文件是否正常的线程 iid："+iid);
        SSThreadCache.delSSChecckDataThread(iid);//关闭这个检测文件是否正常的线程的缓存
