...
//    	return strDay + " " + strHour + ":" + strMinute + ":" + strSecond + " " + strMilliSecond;
    ...
    public static long getDaymillisecond(String time){
    	long l=0;
    	if(time.contains("天")){
    		//LogUtil.intoLog(this.class,"yes");
    		String tian=time.substring(0, time.indexOf("天"));
    		if(null!=tian&&!"".equals(tian)){
    			l = Long.valueOf(tian).longValue();
    			l=l*24*60*60*1000;
    			//LogUtil.intoLog(this.class,"--天-----"+l);
    			//LogUtil.intoLog(this.class,longToTime_all(9335000));
    		}
    		...
    		if(times.contains(":")){
    			...
    			for (int i = 0; i < timelist.length; i++) {
    				if(i==0){
    					...
    					l+=hour;
    					//LogUtil.intoLog(this.class,"--H--"+l);
    				}else if(i==1){
    					long minute =Long.valueOf(timelist[i]).longValue();
    					minute=minute*60*1000;
    					l+=minute;
    					//LogUtil.intoLog(this.class,"--m--"+l);
    				}else if(i==2){
    					long second =Long.valueOf(timelist[i]).longValue();
    					second=second*1000;
    					l+=second;
    					//LogUtil.intoLog(this.class,"---s-"+l);
    				}
        			//LogUtil.intoLog(this.class,"-timelist[i]-----"+i+"------"+timelist[i]);
    			}
    		}
    		//LogUtil.intoLog(this.class,"-------"+times);
    		
    	}else{
    		//LogUtil.intoLog(this.class,"NO");
    		if(time.contains(":")){
    	...
    	//LogUtil.intoLog(this.class,longToTime_all(l));
		return l;
    }
